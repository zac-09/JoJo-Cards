...
const AddNewBirthdayScreen = (props) => {
  ...
  const addBirthdayHandler = useCallback(() => {
    if (editedBirthday) {
      ...
      console.log("day is",newDate.format('D'))
      newDate.set("year",moment.format('YYY'))
      console.log("the new year is ",newDate.format('YYY'))
      console.log("monthis",newDate.format('M'))

      const daysLeft = newDate.diff(today, "days");
      console.log("these are the days left", daysLeft);
