...
import { View, Text, StyleSheet, TouchableWithoutFeedback, Platform, FlatList } from 'react-native';
...
const BirthdayIndex = (props) => {
	const [modalVisible, setIsModalVisible] = useState(props.navigation.getParam('modal') ? false : false);
	const Birthdays = useSelector((state) => state.birthdays.birthdays);
	console.log(Birthdays);
	useEffect(() => {
		setIsModalVisible(false);
	}, []);
	if (Birthdays.length === 0 || Birthdays == null) {
		return (
			<View style={styles.NoContent}>
				<Text>No birthdays found please add some</Text>
				{modalVisible && (
