...
const ConfigureWedddingScreen = (props) => {
	...
	const wedding = props.navigation.getParam('wedding');
	const [modal, setModal] = useState(false);
	const dispatch = useDispatch();
	const deleteHandler = (id) => {
		Alert.alert('Are you sure?', 'Do you really want to delete this wedding?', [
			{ text: 'No', style: 'default' },
			{
				text: 'yes',
				style: 'cancel',
				onPress: () => {
					dispatch(weddingActions.deleteBirthday(id));
		...
		<ScrollView>
			<View style={styles.screen}>
				...
				<View style={styles.content}>
					...
					<Text style={{ ...styles.text, fontFamily: 'open-sans-bold' }}>{wedding.name}</Text>
					<Text style={{ ...styles.text, fontFamily: 'open-sans-bold' }}>{wedding.date}</Text>
					<Text style={styles.text}>Wedding is in {wedding.daysLeft} days</Text>
				</View>
