...
const pushReminder = async () => {
  const localNotification = {
    ...
    body: "its zac' s birthday", // (string) — body text of the notification.
    ios: {
  ...
  const schedulingOptions = {
    time: new Date().getTime() + 3000, // (date or number) — A Date object representing when to fire the notification or a number in Unix epoch time. Example: (new Date()).getTime() + 1000 is one second from now.
    // repeat: repeat,
  };
