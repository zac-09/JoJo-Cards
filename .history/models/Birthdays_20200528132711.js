...
class Birthday {
	async constructor(name, date, phoneNumber, avatar) {
		const today = moment();
		...
		this.zodiac = this.find_zodiac(parseInt(newDate.format('M')), parseInt(newDate.format('D')));
		this.month = month;
		this.pushReminder(name, newDate);
